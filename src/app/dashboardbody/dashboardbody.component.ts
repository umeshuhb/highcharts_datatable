...
export class DashboardbodyComponent implements OnInit {
   ...
   chartOptions1:any={
      ...
      xAxis: {
         ...
         labels: {
            formatter: function() {
            var temp_gmt=new Date(this.value);
            var d = new Date(temp_gmt),
            month = '' + (d.getMonth() + 1),
            day = '' + d.getDate(),
            year = d.getFullYear();   
            if (month.length < 2)
   ...
   chartOptions2:any ={
     ...
     PieClick(event):any{
         let res=LanguageUsers.filter(item=>item.language==event.point.name);
